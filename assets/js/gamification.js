...
(function () {
  ...
  const LOGROS = [
    ...
    { id: 'puntaje-10',   icon: '⭐', nombre: 'Puntaje perfecto',   desc: 'Sacaste 90% o más en un quiz',                check: s => s.quizMejor >= 90 },
    ...
    { id: 'experto-3',    icon: '👑', nombre: 'Nivel Experto dominado', desc: 'Aprobaste 3 quizzes en dificultad Experto', check: s => s.quizAprobadosExperto >= 3 },
  ];
  ...
  const NIVELES_QUIZ = ['facil', 'normal', 'dificil', 'experto'];

  function obtenerStatsPorNivel() {
    const porNivel = {};
    NIVELES_QUIZ.forEach(n => {
      porNivel[n] = {
        jugados:   num(`quiz_${n}_jugados`),
        aprobados: num(`quiz_${n}_aprobados`),
        mejor:     num(`quiz_${n}_mejor`),
      };
    });
    return porNivel;
  }
  ...
  function obtenerStats() {
    ...
    const porNivel = obtenerStatsPorNivel();
    return {
      ...
      quizPorNivel: porNivel,
      quizAprobadosExperto: porNivel.experto.aprobados,
    };
  }
  ...
  const LLAVES_PROGRESO = [
    ...
    'quiz_facil_jugados', 'quiz_facil_aprobados', 'quiz_facil_mejor',
    'quiz_normal_jugados', 'quiz_normal_aprobados', 'quiz_normal_mejor',
    'quiz_dificil_jugados', 'quiz_dificil_aprobados', 'quiz_dificil_mejor',
    'quiz_experto_jugados', 'quiz_experto_aprobados', 'quiz_experto_mejor',
  ];
